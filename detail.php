...
// Recuperation et validation de l'id passe en URL (?id=X)
$id = intval($_GET['id'] ?? 0);
...
// Recherche de la declaration en base
$requete = $pdo->prepare("SELECT * FROM declarations WHERE id = :id");
$requete->execute([':id' => $id]);
$declaration = $requete->fetch();
...
<table>
    ...
    <tr>
        <td>Enregistree le</td>
        <td><?= date('d/m/Y \a H:i', strtotime($declaration['created_at'])) ?></td>
    </tr>
</table>
